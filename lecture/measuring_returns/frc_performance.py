...
from data.readers import trackers_frc
...
df = trackers_frc()
df.columns = df.columns.str.replace("FRC ", "")
...
plt.savefig(figure_path.joinpath("Measuring Returns - FRC Performance.pdf"))
plt.show()
plt.close()
...
plt.savefig(figure_path.joinpath("Measuring Returns - FRC Performance Table.pdf"))
plt.show()
plt.close()
