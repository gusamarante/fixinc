...
for t in dates2loop:
    ...
    w = 1 / duration
    bnss = BootstrapNSS2(prices, all_cashflows, w, t, dc, beta0=beta0_run, lam0=lam0_run, verbose=False, alpha_lam=0.0001, alpha_beta=0.00001)
    print(f"Date: {t:%Y-%m-%d}, SSE={bnss.sse:.6f}, Beta={bnss.beta}, Lambda={bnss.lam}, n_bonds={len(prices)}, min_mat={aux_raw['du'].min()}")
...
#     all_cashflows = pd.concat(all_cashflows, axis=1).fillna(0)
#     w = 1 / duration
#     bnss = BootstrapNSS2(prices, all_cashflows, w, t, dc, beta0=beta0_new, lam0=lam0_new, verbose=False)
#
#     if bnss.sse < row["sse"]:
#         print(f"Date: {t:%Y-%m-%d}, SSE improved: {row['sse']:.6f} -> {bnss.sse:.6f}")
#         con.execute(
#             "INSERT OR REPLACE INTO nss_parameters VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
#             (CURVE_ID, t.date().isoformat(), *bnss.beta, *bnss.lam, bnss.sse)
#         )
#         con.commit()
#     else:
#         print(f"Date: {t:%Y-%m-%d}, no improvement (old={row['sse']:.6f}, new={bnss.sse:.6f})")
