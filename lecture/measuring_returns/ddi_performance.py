...
from data.readers import trackers_ddi
...
df = trackers_ddi()
df.columns = df.columns.str.replace("DDI ", "")
...
plt.savefig(figure_path.joinpath("Measuring Returns - DDI Performance.pdf"))
plt.show()
plt.close()
...
plt.savefig(figure_path.joinpath("Measuring Returns - DDI Performance Table.pdf"))
plt.show()
plt.close()
