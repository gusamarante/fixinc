"""
Builds total return indexes for NTN-Bs and saves them in a CSV file. This
routine only works on my computer.
"""
...
for dd in desired_duration:
    ...
    aux_data = aux_data[aux_data['du'] >= 21 * rebalance_window + 1]
    ...
    for date, datem1 in tqdm(paired_dates, f'Backtesting LTN-NTNF {dd}y'):
        ...
        if date < next_rebalance_date:  # still behind the rebalance, MtM
            ...
            aux_data_select = aux_data[aux_data['du'] >= 21 * rebalance_window + 1]
    ...
    # Standardize the trackers
    df_bt = 100 * df_bt['Notional'] / df_bt['Notional'].iloc[0]
    df_bt = df_bt.rename(f'LTN NTNF {dd}y')
    df_tracker = pd.concat([df_tracker, df_bt], axis=1)


# ===== Save Trackers =====
df_tracker.to_csv(data_output.joinpath('trackers_ltn_ntnf.csv'))
