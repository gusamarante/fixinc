from fixinc import Performance, SGS
from data.readers import trackers_ltn_ntnf
...
df = trackers_ltn_ntnf()
df.columns = df.columns.str.replace("LTN NTNF ", "")
...
plt.savefig(figure_path.joinpath("NTNF - Performance.pdf"))
plt.show()
plt.close()
...
plt.savefig(figure_path.joinpath("NTNF - Performance Table.pdf"))
plt.show()
plt.close()
